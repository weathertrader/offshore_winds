...
#     (stn_metadata_dict) = read_stn_metadata_from_csv(dir_work, location, use_stn, print_stn_info) 
# version history 
...
def read_stn_metadata_from_csv(dir_work, location, use_stn, print_stn_info):
    ...
    stn_name           = stn_read_df_matrix[:, 0] 
    stn_id             = stn_read_df_matrix[:, 1] 
    stn_lon            = stn_read_df_matrix[:, 2] 
    stn_lat            = stn_read_df_matrix[:, 3] 
    stn_ele            = stn_read_df_matrix[:, 4] 
    stn_obs_hgt        = stn_read_df_matrix[:, 5] 
    stn_mnet_id        = stn_read_df_matrix[:, 6] 
    stn_tmzn           = stn_read_df_matrix[:, 7] 
    stn_datetime_start = stn_read_df_matrix[:, 8] 
    stn_datetime_end   = stn_read_df_matrix[:, 9] 
    #stn_state          = stn_read_df_matrix[:,10] 
    #stn_county         = stn_read_df_matrix[:,11] 
    #stn_cwa            = stn_read_df_matrix[:,12] 
    #stn_notes          = stn_read_df_matrix[:,13] 
    n_stn = len(stn_lat)
    
    del file_name_stn_read, stn_read_csv, stn_read_df_matrix 
    ...
    stn_metadata_dict['stn_datetime_start'] = stn_datetime_start
    stn_metadata_dict['stn_datetime_end']   = stn_datetime_end
